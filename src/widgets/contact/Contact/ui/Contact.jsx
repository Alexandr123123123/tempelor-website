      ...
      <div className={styles.container}>
        ...
        >
          <div className={styles.label}>
            <span className={styles.accentSlash}>//</span> {t("contact.label")}
          </div>
          ...
          <motion.div className={styles.infoWrapper} variants={containerVariants}>
            ...
            >
              ...
              <div className={styles.bentoContent}>
                ...
                <p>{t("contact.officeAddress1")}<br />{t("contact.officeAddress2")}</p>
              </div>
            ...
            <AnimatePresence mode="wait">
              {isSuccess ? (
                ...
                >
                  ...
                  <h3 style={{ marginBottom: '8px', color: '#fff', fontSize: '1.5rem' }}>{t("contact.successTitle")}</h3>
                  <p style={{ color: 'rgba(255,255,255,0.7)' }}>{t("contact.successText")}</p>
                </motion.div>
              ) : (
                <motion.form 
                  key="form"
                ...
                >
                  <div className={styles.formRow}>
                    <div className={styles.inputGroup}>
                      <label htmlFor="firstName">{t("contact.formFirstName")}</label>
                      <input type="text" id="firstName" value={formData.firstName} onChange={handleInputChange} required placeholder={t("contact.formFirstNamePh")} />
                    </div>
                    <div className={styles.inputGroup}>
                      <label htmlFor="lastName">{t("contact.formLastName")}</label>
                      <input type="text" id="lastName" value={formData.lastName} onChange={handleInputChange} placeholder={t("contact.formLastNamePh")} />
                    </div>
                  </div>
                  <div className={styles.inputGroup}>
                    <label htmlFor="email">{t("contact.formEmail")}</label>
                    <input type="email" id="email" value={formData.email} onChange={handleInputChange} required placeholder={t("contact.formEmailPh")} />
                  </div>
                  <div className={styles.inputGroup}>
                    <label htmlFor="message">{t("contact.formMessage")}</label>
                    <textarea id="message" value={formData.message} onChange={handleInputChange} required rows="4" placeholder={t("contact.formMessagePh")}></textarea>
                  </div>
