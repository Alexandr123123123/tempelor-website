...
export const ServicesGrouped = () => {
  ...
  const blocks = [
    ...
    },
    {
      ...
      themeColor: "#00ff80",
      themeRgb: "0, 255, 128",
      themeAccentRgb: "0, 195, 255",
      categories: [
        {
          id: "smart",
          label: "Smart Home",
          bgImage: '/smart_home_monitor.png',
          icon: (
            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      ...
      themeColor: "#00c3ff",
      themeRgb: "0, 195, 255",
      themeAccentRgb: "128, 0, 255",
      categories: [
        {
          id: "backup",
          label: "Backup Power",
          bgImage: "/backup_bg.png",
          icon: (
            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className={styles.container}>
      {blocks.map((block) => {
        const isSingleCard = block.categories.length === 1;

        return (
          <motion.div 
            key={block.id} 
            className={`${styles.neonBlock} ${isSingleCard ? styles.neonBlockHorizontal : ''}`}
            style={{ 
              '--theme-color': block.themeColor, 
              '--theme-rgb': block.themeRgb,
              '--theme-accent-rgb': block.themeAccentRgb 
            }}
          ...
          >
            ...
            <div className={`${isSingleCard ? styles.horizontalCard : styles.grid} ${block.categories.length === 2 ? styles.gridTwoCols : ''}`}>
              {block.categories.map((cat) => (
                <motion.div 
                  key={cat.id} 
                  className={`${styles.card} ${cat.bgImage ? styles.cardWithBg : ''}`}
                  style={cat.bgImage ? {
                    backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.65), rgba(0, 0, 0, 0.8)), url(${cat.bgImage})`,
